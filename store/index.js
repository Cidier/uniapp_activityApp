...
export default new Vuex.Store({
	state: {
		...
		// 当前聊天对象（进入聊天页面获取）
		ToUser:{
			user_id:0, // 通过user_id 是否为0，判断用户处在什么场景下
			username:"",
			userpic:""
		},
		// 聊天会话列表
	...
	mutations:{
		...
		editUserUserInfo(state, obj) {
			if (state.user.userinfo) {
				state.user.userinfo.name = obj.name
	...
	actions: {
		...
		async updateChatList({state, dispatch, commit}, {data, send}) {
			...
			// 是否本人发送
			let isMySend = send
			console.log(isMySend? "是本人发送信息": "不是本人发送的")
			// 获取之前会话
			let chatList = await dispatch("getChatList")
			// 判断是否已经存在该会话，存在：将会话置顶； 不存在：创建并追加到头部
			...
			if (i === -1) {
				...
				// 当前聊天对象是否是该Id,未读数+1，（排除本人发送信息）
				if (!isMySend && stata.ToUser.user_id !== item.user_id) {
					item.noread++
				}
		...
		formatSendData({state},data) {
			return {
				...
				from_userpic: state.user.userpic ? state.user.userpic : "/static/1.jpg",
				type: data.type,
				data: data.data,
